...
- `COMPATRA_COMPAT_MAX_COMMAND_BYTES=1048576`: maximum length read for a guest `system()`/`popen()` command string before it is treated as truncated; default is 1 MiB, matching macOS's own `ARG_MAX`. Truncated commands are logged with `Truncated: true` and rejected with `E2BIG` instead of being silently cut and executed anyway.
