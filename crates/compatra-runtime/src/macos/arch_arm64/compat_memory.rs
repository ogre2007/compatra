...
impl compatra::GuestMemory for Arm64CompatGuestMemory<'_> {
    fn read_memory(
        &mut self,
        addr: u64,
        size: usize,
    ...
    fn allocate_memory(
        ...
        size: usize,
        alignment: usize,
    ) -> Result<u64, compatra::GuestMemoryError> {
        allocate_arm64_heap(
            self.emulator,
        ...
        .map(|(addr, _)| addr)
        .ok_or(compatra::GuestMemoryError)
    }

    fn free_memory(&mut self, addr: u64) -> Result<(), compatra::GuestMemoryError> {
    ...
    fn set_guest_program_name_ptr(&mut self, addr: u64) -> Result<(), compatra::GuestMemoryError> {
