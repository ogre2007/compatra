...
fn debug_stdout_enabled() -> bool {
    std::env::var("COMPATRA_DEBUG_STDOUT")
        .ok()
        .map(|v| {
            let v = v.trim();
            v == "1"
...
/// `COMPATRA_PROFILE` knob lets analysts opt into longer budgets without
...
///
/// Explicit `COMPATRA_TIMEOUT_USECS` / `COMPATRA_MAX_INSTRUCTIONS` always win.
fn resolve_run_profile() -> (&'static str, u64, usize) {
    let raw = std::env::var("COMPATRA_PROFILE").ok();
...
pub fn install_arm64_diagnostic_hooks(
    ...
    emulator.add_code_hook(
        actual_entry,
        done_addr + 4,
        move |emu: &mut compatra_runtime::UnicornEmulator, address: u64, size: u32| {
    ...
    if let (Some(window_start), Some(window_end)) = (
        env_hex_u64("COMPATRA_TRACE_WINDOW_START"),
        env_hex_u64("COMPATRA_TRACE_WINDOW_END"),
    ) {
        let max_hits = env_usize("COMPATRA_TRACE_WINDOW_HITS", 128);
    ...
    if env_bool("COMPATRA_AUTH_DISPATCH_DIAG") {
        ...
        let hit_limit = env_usize("COMPATRA_AUTH_DISPATCH_HITS", 128);
...
pub fn run_arm64_with_diagnostics(
    ...
    let timeout_usecs = env_u64("COMPATRA_TIMEOUT_USECS", profile_timeout);
    let instruction_count = env_usize("COMPATRA_MAX_INSTRUCTIONS", profile_instructions);
    {
        let metadata = runtime_process_metadata(report.process_name.clone());
