            ...
            if let Some(load) = decode_ldr_uimm64(prev) {
                if load.rt == reg {
                    ...
                    emulator.add_code_hook(
                        load_addr,
                        load_addr + 4,
                        move |emu: &mut compatra_runtime::UnicornEmulator,
                              address: u64,
                              _size: u32| {
...
fn read_arm64_addr_reg(emu: &mut compatra_runtime::UnicornEmulator, reg: u8) -> Option<u64> {
    if reg == 31 {
