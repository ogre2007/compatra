...
fn synthesize_missing_open_target(
    ...
    if guest_artifacts::path_looks_like_directory(raw_path) {
        ...
        table.directories.insert(
            ...
            SyntheticGuestDirectory {
                ...
                entries: guest_artifacts::synthetic_directory_entries(raw_path)
                    .into_iter()
        ...
        table.files.insert(
            ...
            SyntheticGuestFile {
                raw_path: raw_path.to_string(),
                resolved_path: resolved.to_path_buf(),
                kind: SyntheticGuestFileKind::HostBytes(
                    guest_artifacts::materialize_synthetic_file_bytes(
                        raw_path,
...
pub fn open_guest_path_with_flags(
    ...
    let allow_synthesize = table.policy.materialize_missing_paths
        && guest_artifacts::should_materialize_missing_path(raw_path);
...
pub fn stat_guest_path(table: &GuestFileTable, raw_path: &str) -> Result<(u64, PathBuf), u32> {
    ...
    match raw_path {
        ...
        _ => match std::fs::metadata(&resolved) {
            ...
            {
                Ok((
                    guest_artifacts::synthetic_path_size(
                        raw_path,
                        table.policy.synthetic_file_size,
                    ),
                    resolved,
...
mod tests {
    ...
    fn table() -> GuestFileTable {
        GuestFileTable::new(std::env::temp_dir().join("compatra-guest-files-test"))
    }
    ...
    fn compat_policy_does_not_materialize_missing_analysis_artifacts() {
        let mut table = GuestFileTable::with_policy(
            std::env::temp_dir().join("compatra-guest-files-compat-test"),
