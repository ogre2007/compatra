...
mod tests {
    ...
    impl Drop for CaptureEnvGuard {
        fn drop(&mut self) {
            if let Some(old) = self.old.take() {
                std::env::set_var("COMPATRA_PAYLOAD_DUMP_DIR", old);
            } else {
                std::env::remove_var("COMPATRA_PAYLOAD_DUMP_DIR");
            }
    ...
    fn temp_capture_env(label: &str) -> CaptureEnvGuard {
        ...
        let old = std::env::var_os("COMPATRA_PAYLOAD_DUMP_DIR");
        std::env::set_var("COMPATRA_PAYLOAD_DUMP_DIR", &dir);
        CaptureEnvGuard {
            _lock: lock,
            old,
            dir,
