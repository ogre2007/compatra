...
//! "fast" indirect-branch mode (`COMPATRA_INDIRECT_BRANCH_MODE` unset). If
...
//!
//! The test is run-once: it spawns one `machoscope` subprocess, captures its
...
fn rustdoor_fast_mode_reaches_log_stream_posix_spawnp() {
    ...
    let output = Command::new(machoscope_binary())
        ...
        .env("COMPATRA_INDIRECT_BRANCH_MODE", "fast")
