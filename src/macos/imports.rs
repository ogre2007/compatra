...
fn log_detection(category: &str, detail: impl AsRef<str>) {
    println!("[DETECT][{}] {}", category, detail.as_ref());
}
...
fn detect_import_activity(symbol: &str) {
    match symbol {
        "fork" => log_detection("process", "fork import called"),
        "ptrace" => log_detection("anti-debug", "ptrace import called"),
        "setsid" => log_detection("process", "setsid import called"),
        "setuid" | "setgid" | "setgroups" => {
            log_detection("privilege", format!("{} import called", symbol))
        }
        "execve" => log_detection("process", "execve import called"),
        "kill" | "killpg" | "raise" | "pthread_kill" => {
            log_detection("signal", format!("{} import called", symbol))
        }
        _ => {}
    }
}
...
pub fn initialize_fake_x64_import_data(
    ...
    emulator.write_memory(fake_data_addr + 0x190, b"machina-host\0")?;
...
pub fn default_x64_import_policies(fake_data_addr: u64) -> Vec<(String, ImportReturnPolicy)> {
    ...
    policies.extend([
        ...
        (
            ...
            ImportReturnPolicy::WriteCStringToArg0(b"machina-host\0"),
        ),
        (
...
fn emulate_synthetic_fork(state: &mut ImportThreadState) -> u64 {
    ...
    log_detection(
        "process",
        format!(
            "synthetic fork observed parent_pid={} child_pid={}",
            SYNTHETIC_PARENT_PID, pid
        ),
    );
    pid
...
fn mark_synthetic_process_exit(state: &mut ImportThreadState, pid: u64, exit_status: i32) {
    if let Some(proc) = state.synthetic_processes.get_mut(&pid) {
        ...
        log_detection(
            "process",
            format!(
                "synthetic child pid={} exited status={}",
                proc.pid, proc.exit_status
            ),
        );
    }
}
...
pub fn install_x64_import_dispatcher(
    ...
    for (addr, thunk) in import_thunks {
        ...
        emulator.add_code_hook(addr, addr + 1, move |emu, _addr, _size| {
            ...
            detect_import_activity(&thunk.symbol);
