...
use crate::macos::guest_analysis_artifacts;
...
fn synthesize_missing_open_target(
    ...
    if guest_analysis_artifacts::path_looks_like_directory(raw_path) {
        ...
        table.directories.insert(
            ...
            SyntheticGuestDirectory {
                ...
                entries: guest_analysis_artifacts::synthetic_directory_entries(raw_path),
            },
        );
        ...
        table.files.insert(
            ...
            SyntheticGuestFile {
                ...
                kind: SyntheticGuestFileKind::HostBytes(
                    guest_analysis_artifacts::materialize_synthetic_file_bytes(
                        raw_path,
...
pub fn open_guest_path_with_flags(
    ...
    let allow_synthesize = table.policy.materialize_missing_paths
        && guest_analysis_artifacts::should_materialize_missing_path(raw_path);
...
pub fn stat_guest_path(table: &GuestFileTable, raw_path: &str) -> Result<(u64, PathBuf), u32> {
    ...
    match raw_path {
        ...
        _ => match std::fs::metadata(&resolved) {
            ...
            {
                Ok((
                    guest_analysis_artifacts::synthetic_path_size(raw_path, &table.policy),
                    resolved,
                ))
            }
            Err(_) => Err(2),
