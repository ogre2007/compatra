...
pub fn create_default_syscall_handler(
    ...
    Box::new(move |emu: &mut dyn Emulator| -> Result<i64, MacOsError> {
        ...
        match syscall_number {
            SYSCALL_SETUID => log_detection("privilege", format!("setuid syscall uid={}", args[0])),
            SYSCALL_KILL => log_detection(
                "signal",
                format!("kill syscall pid={} sig={}", args[0], args[1]),
            ),
            SYSCALL_EXIT | SYSCALL_EXIT_GROUP => log_detection(
                "process",
                format!("process exit requested code={}", args[0]),
            ),
            _ => {}
        }
